...
class SignInForm extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={`${styles.signinblock} ${this.props.status ? styles.frontblock : ""} ${
          this.props.btn
        }`}
      >
        <div className={`${styles.instructlogin}`}>
          ...
          <motion.button whileTap={{ scale: 1.3 }} onClick={this.handleButton}>
            Sign In
          </motion.button>
        </div>
          ...
          <form className={styles.signupform} onSubmit={this.handleSubmit}>
            <div className={`${styles.icon}`}>
              ...
              <input
                ...
                className={`${styles.takeInput} ${
                  unameError.length > 0 ? styles["error"] : styles["applyBorder"]
                }`}
              />
              {unameError.length > 0 ? <span className={styles.msgError}>{unameError}</span> : null}
            </div>
            ...
            <div className={`${styles.icon}`}>
              ...
              <input
                ...
                className={`${styles.takeInput} ${
                  emailError.length > 0 ? styles["error"] : styles["applyBorder"]
                }`}
              />
              {emailError.length > 0 ? <span className={styles.msgError}>{emailError}</span> : null}
            </div>
            <div style={{ margin: "5px 0", width: "70%" }}>
              ...
              {contactError.length > 0 ? (
                <span className={styles.msgError}>{contactError}</span>
              ) : null}
            </div>
            ...
            <div className={`${styles.icon}`}>
              ...
              <input
                ...
                className={`${styles.takeInput} ${
                  passwordError.length > 0 ? styles["error"] : styles["applyBorder"]
                }`}
              />
              {passwordError.length > 0 ? (
                <span className={styles.msgError}>{passwordError}</span>
              ) : null}
            </div>

            <button type='submit'>Sign Up</button>
            {isSignedIn || isSignedInFromGoogle ? (
              <Redirect
                to={{
                  pathname: "/dashboard",
                  state: {
