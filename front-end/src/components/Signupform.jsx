...
class SignUpForm extends Component {
  ...
  handleSubmit = (event) => {
    ...
    if (formValid(errors)) {
      ...
      axios.post(login, userDetails).then(
        (res) => {
          ...
          <form className={styles.signinform} onSubmit={this.handleSubmit}>
            <div className={`${styles.icon}`}>
              ...
              <input
                ...
                className={`${styles.takeInput} ${
                  eError.length > 0 ? styles.error : styles.applyBorder
                }`}
              />
              {eError.length > 0 ? <span className={styles.msgError}>{eError}</span> : null}
            </div>
            ...
            <div className={`${styles.icon}`}>
              ...
              <input
                ...
                className={`${styles.takeInput} ${
                  passError.length > 0 ? styles["error"] : styles["applyBorder"]
                }`}
              />
              {passError.length > 0 ? <span className={styles.msgError}>{passError}</span> : null}
            </div>
            ...
            <FooterLogIn
              ...
              signInStatus={this.state.isSignedInFromGoogle}
            />
          </form>
        </div>
      </div>
